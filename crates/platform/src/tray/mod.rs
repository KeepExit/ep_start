...
impl TrayIcon {
	pub fn create( config: TrayIconConfig, handler: impl FnMut( TrayEvent ) + 'static ) -> Result< Self, String > {
		...
		let state = Box::new( TrayState { hwnd: HWND::default(), icon, tooltip: config.tooltip, menu_entries: config.menu, handler: Box::new( handler ), taskbar_created_message, icon_added: false } );
