...
pub( crate ) fn draw_setting_row( painter: &Painter, theme: &SettingsTheme, row: &SettingRowLayout, view: SettingView< '_ >, active_slider: Option< SettingId > ) {
	...
	match row.kind {
		ControlKind::Slider => draw_slider_control( painter, theme, row.id, row.control, &view.value, view.minimum, view.maximum, view.ratio, active_slider.is_some_and( |field| field == row.id ) ),
