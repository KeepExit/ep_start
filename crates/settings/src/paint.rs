...
use crate::ui::components::{ SettingView, draw_action_button, draw_setting_row, draw_sidebar_item };
...
fn paint_settings_page( hdc: HDC, client: RECT, dpi: i32, view: SettingsPageView< '_ > ) {
	...
	for section in &layout.content.sections {
		...
		for item in &section.items {
			draw_setting_row( &painter, view.theme, item, setting_view( view.text, view.preferences, item.id ), view.active_slider );
		}
	}
	...
	for action in &layout.footer.actions {
		draw_action_button( &painter, view.theme, action.area, action_label( view.text, action.id ), action.primary, view.dirty );
	}
